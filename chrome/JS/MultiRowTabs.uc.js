...
(function() {
	...
	(function() {
		//TODO: Hierarquia?
		// Se popup ter draover, permite outra ação sem delay. Senão, adiciona delay no fechar e no abrir de outros.

		/*
		- Reqs:
		. Afetar apenas se arrastando item
		. Popup tem delay para abrir e fechar

		- popupshow:
		. dragover em popup e popupParent os marca.
		  Se não tiver marca, popuphide remove os eventos, nada ocorre.
		. popupshow e popuphide cancelam os eventos e iniciam um timer. No final, executam normal.
		  Mas, dragover em popup ou popupParent reinicia o timer.
		  popushow sempre lida apenas com um único popup, o último.
		  popuhide sempre lida apenas com um único popup, o primeiro.
		...
		*/
		...
		const states = {
			...
			"DRAGGING_OVER": (popupElem) => {
				...
				if(closeTimer) clearTimeout(closeTimer);
				onDragOverThisPopup = () => {
					console.log("onDragOverThisPopup");
					states.loadState("DEFAULT", popupElem);
				};
				onPopupShowing = (event) => {
					if(ignore) return;
					...
					openTimer = setTimeout(() => {
						...
						states.loadState("POPUP_OPEN", newPopupElem);
					}, closeDelay);
				};
				onThisPopupHiding = (event) => {
					if(ignore) return;
					console.log("onThisPopupHiding");
					// Impede de fechar
					event.preventDefault();
					event.stopPropagation();
					// Delay
			...
			loadState: (newState, popupElem = null) => {
				...
				onDragOverMenu = () => {};
				onDragOverThisPopup = () => {};
				states[newState](popupElem);
				document.getElementById("PlacesToolbar").addEventListener("popupshowing", onPopupShowing, false);
				if(popupElem) {
					popupElem.addEventListener("popuphiding", onThisPopupHiding, false);
					popupParent.addEventListener("dragover", onDragOverMenu, false);
					popupElem.addEventListener("dragover", onDragOverThisPopup, false);
				}
			}
		};
		// RUN
		...
		document.getElementById("PlacesToolbar").addEventListener("popupshowing", (event) => {
			...
			let onDragOverPopup = () => {
				if(!didDragOnPopup) reset();
				didDragOnPopup = true;
			};
			let onHiding = () => reset();
			let reset = () => {
				popupParent.removeEventListener("dragover", onDragOver);
				popup.removeEventListener("dragover", onDragOver);
				popup.removeEventListener("popuphiding", onHiding);
				didDragOnMenu = false;
				didDragOnPopup = false;
			}
			popupParent.addEventListener("dragover", onDragOverMenu);
			popup.addEventListener("dragover", onDragOverPopup);
			popup.addEventListener("popuphiding", onHiding);
	...
	// console.log(event);
			let popup = event.target;
			let popupParent = event.target.parentElement;
			// Impede outros popups de abrirem
			if(lock) {
				// Impede de abrir
				event.preventDefault();
				event.stopPropagation();
				// Marca para ser aberto se o outro fechar
				onOldMenuClose = () => {
					popup.openPopup();
				};
				return;
			}
			// Na pasta-pai: Anota o tempo em que o deixara
			let onDragOver =  () => {
				popupParent.lastDragTime = Date.now();
			};
			popupParent.addEventListener("dragover", onDragOver);
			// No popup: Marca se entrara nele
			let onDragEnter =  () => {
				popup.dragEntered = true;
				lock = false;
			};
			popup.addEventListener("dragover", onDragEnter);
			// 
			lock = true;
			popup.addEventListener("popuphiding", (event) => {
	// console.log(event);
				if(Date.now() - popupParent.lastDragTime < closeDelay && !popup.dragEntered) { // Impede de fechar antes de delay
					// Impede de fechar
					event.preventDefault();
					event.stopPropagation();
					setTimeout(() => {
						popup.hidePopup();
						popupParent.removeEventListener("dragover", onDragOver);
						popup.removeEventListener("dragover", onDragEnter);
						lock = false;
						onOldMenuClose();
					}, closeDelay);
				} else {
